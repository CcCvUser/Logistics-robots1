...
void uart_rx_idle_interrupt (uart_index_enum uart_n, uint32 status)
{
    if(status)  
    {
//        LPUART_EnableInterrupts(uart_index[uart_n], kLPUART_RxDataRegFullInterruptEnable);
        LPUART_ClearStatusFlags(uart_index[uart_n], kLPUART_IdleLineFlag);
        LPUART_EnableInterrupts(uart_index[uart_n], kLPUART_IdleLineInterruptEnable);
        interrupt_enable((IRQn_Type)(LPUART1_IRQn + (IRQn_Type)(uart_n - 1)));
    }
    else
    {
